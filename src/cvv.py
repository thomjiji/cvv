...
class CopyJob:
    ...
    def _run_and_get_result(self) -> FileCopyResult:
        ...
        try:
            ...
            if self.config.verification_mode != VerificationMode.TRANSFER:
                result.source_hash_inflight = self._run_pipeline(source_size, enable_hashing=True)
            else:
    ...
    def _check_disk_space(self, source_size: int):
        for dest in self._destinations:
            ...
            if usage.free < source_size:
                raise CopyJobError(f"Not enough space on {dest.parent}. Required: {source_size/1e9:.2f}GB, Available: {usage.free/1e9:.2f}GB")
    ...
    def _run_pipeline(self, source_size: int, enable_hashing: bool) -> str | None:
        source_hasher = HashCalculator(self.config.hash_algorithm) if enable_hashing else None
        ...
        for i, dest_path in enumerate(self._destinations):
            thread = threading.Thread(target=self._writer_thread, args=(dest_path, chunk_queues[i]))
            thread.start()
        ...
        try:
            with open(self._source, "rb") as f_src:
                while True:
                    if self._abort_event.is_set(): raise CopyJobError("Operation aborted")
                    chunk = f_src.read(self.config.buffer_size)
                    if not chunk: break
                    if source_hasher:
                        source_hasher.update(chunk)
                    progress.update(len(chunk))
                    for q in chunk_queues: q.put(chunk)
        finally:
            for q in chunk_queues: q.put(None)
    ...
    def _compare_sizes(self, source_size: int):
        for dest in self._destinations:
            ...
            if source_size != dest_size:
                raise CopyJobError(f"File size mismatch for {dest.name}. Source: {source_size}, Dest: {dest_size}")

    def _verify_source_post_copy(self, result: FileCopyResult):
        logging.info(f"Re-hashing source file {self._source.name}...")
        post_hash = HashCalculator.hash_file(self._source, self.config.hash_algorithm, self.config.buffer_size)
        result.source_hash_post = post_hash
        if post_hash != result.source_hash_inflight:
            result.verified = False
            raise CopyJobError(f"Source file changed during copy. In-flight: {result.source_hash_inflight}, Post-copy: {post_hash}")
    ...
    def _verify_full_post_copy(self, result: FileCopyResult):
        ...
        with ThreadPoolExecutor(max_workers=len(files_to_hash)) as executor:
            future_to_path = {executor.submit(HashCalculator.hash_file, path, self.config.hash_algorithm, self.config.buffer_size): path for path in files_to_hash}
        ...
        if result.source_hash_post != result.source_hash_inflight:
            result.verified = False
            raise CopyJobError(f"Source file changed during copy. In-flight: {result.source_hash_inflight}, Post-copy: {result.source_hash_post}")

        for dest in self._destinations:
            if result.destination_hashes_post[dest] != result.source_hash_inflight:
                result.verified = False
                logging.error(f"HASH MISMATCH: {dest.name} hash {result.destination_hashes_post[dest]} != source hash {result.source_hash_inflight}")
...
class BatchProcessor:
    ...
    def _execute_jobs(self, source_files: list[Path]):
        ...
        for i, file_path in enumerate(source_files):
            ...
            for dest in dest_paths: job.add_destination(dest)
    ...
    def _process_results(self) -> list[FileCopyResult]:
        results = []
        for job, future in self.jobs:
            try:
                result = future.result() # Blocks until this job is done
                sys.stdout.write("\n")
                if result.error:
                    logging.error(f"Result for {result.source_path.name}: FAILED ({result.error})")
                else:
                    logging.info(f"Result for {result.source_path.name}: SUCCESS")
                    logging.info(f"  - Speed: {result.speed_mb_sec:.2f} MB/s, Size: {result.size/1e6:.2f} MB")
                    if result.source_hash_inflight:
                        logging.info(f"  - Hash ({self.config.hash_algorithm}): {result.source_hash_inflight}")
    ...
    def _final_summary(self, results: list[FileCopyResult]) -> bool:
        ...
        if success_count == total_count:
            ...
            logging.error(f"{total_count - success_count} out of {total_count} operations failed.")
            return False
...
def main():
    ...
    parser.add_argument("-m", "--mode", type=str, default="full", choices=["transfer", "source", "full"], help="Verification mode.")
    parser.add_argument("--hash-algorithm", type=str, default="xxh64be", choices=["xxh64be", "md5", "sha1", "sha256"], help="Hash algorithm for verification.")
